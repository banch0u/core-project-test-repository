...
const QuestionnairesDocumentTypeContent = () => {
  ...
  const { loading, educationlevelsRender } = useSelector(
    (state) => state.global
  );

  const educationlevels = useSelector(
    (state) => state.questionnaire.educationlevels
  );
  const paginationLength = setPaginationLength(
    educationlevels?.count,
    educationlevels?.size
  );

  const onSubmit = useCallback(
    async (data) => {
      dispatch(addEducationlevels({ ...data, category: typeSelect }));
    },
    [dispatch, typeSelect]
  );
  const onEdit = useCallback(
    (id, record) => {
      ...
      dispatch(editEducationlevels(data));
    },
    [dispatch, typeSelect]
  );
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(educationlevelsVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (educationlevels?.items) {
    data = educationlevels?.items?.map((dataObj, i) => ({
      num:
        educationlevels?.size * educationlevels?.page +
        i +
        1 -
        educationlevels?.size,
      id: dataObj?.id,
  ...
  useEffect(() => {
    ...
    dispatch(getEducationlevels(data));
  }, [dispatch, page, educationlevelsRender, typeSelect, size, query]);
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteEducationlevels(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
