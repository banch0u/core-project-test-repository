...
const { Item } = Form;
const QuestionnairesIssuedAuthoritiesContent = () => {
  ...
  const [size, setSize] = useState(
    Cookies.get("pagination-size-questionnaire-issued-authorities")
      ? JSON.parse(
          Cookies.get("pagination-size-questionnaire-issued-authorities")
        )
      : 20
  ...
  const { loading, issuedauthoritieRender } = useSelector(
    (state) => state.global
  );

  const issuedauthoritie = useSelector(
    (state) => state.questionnaire.issuedauthoritie
  );
  const paginationLength = setPaginationLength(
    issuedauthoritie?.count,
    issuedauthoritie?.size
  );

  const onSubmit = useCallback(
    async (data) => {
      dispatch(addIssuedauthoritie(data));
    },
    [dispatch]
  );
  const onEdit = useCallback(
    (id, record) => {
      ...
      dispatch(editIssuedauthoritie(data));
    },
    [dispatch]
  );
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(issuedauthoritiesVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (issuedauthoritie?.items) {
    data = issuedauthoritie?.items?.map((dataObj, i) => ({
      num:
        issuedauthoritie?.size * issuedauthoritie?.page +
        i +
        1 -
        issuedauthoritie?.size,
      id: dataObj?.id,
  ...
  useEffect(() => {
    ...
    dispatch(getIssuedauthoritie(data));
  }, [dispatch, page, issuedauthoritieRender, query, size]);
  const updateSize = (newSize) => {
    setSize(newSize); // Update state
    Cookies.set(
      "pagination-size-questionnaire-issued-authorities",
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteIssuedauthoritie(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
...
export default QuestionnairesIssuedAuthoritiesContent;
