...
const { Item } = Form;
const QuestionnairesDeliveryMethodContent = () => {
  ...
  const [size, setSize] = useState(
    Cookies.get("pagination-size-quetionnaire-delivery-method")
      ? JSON.parse(Cookies.get("pagination-size-quetionnaire-delivery-method"))
      : 20
  );
  const [query, setQuery] = useState({ name: "" });
  const { loading, deliveryMethodRender } = useSelector(
    (state) => state.global
  );

  const deliveryMethod = useSelector(
    (state) => state.questionnaire.deliveryMethod
  );
  ...
  const onEdit = useCallback(
    (id, record) => {
      ...
      dispatch(editDeliveryMethod(data));
    },
    [dispatch]
  );
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(deliveryMethodVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (deliveryMethod?.items) {
    data = deliveryMethod?.items?.map((dataObj, i) => ({
      num:
        deliveryMethod?.size * deliveryMethod?.page +
        i +
        1 -
        deliveryMethod?.size,
      id: dataObj?.id,
  ...
  useEffect(() => {
    ...
    dispatch(getDeliveryMethod(data));
  }, [dispatch, page, deliveryMethodRender, query, size]);
  const updateSize = (newSize) => {
    setSize(newSize); // Update state
    Cookies.set(
      "pagination-size-quetionnaire-delivery-method",
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteDeliveryMethod(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
...
export default QuestionnairesDeliveryMethodContent;
