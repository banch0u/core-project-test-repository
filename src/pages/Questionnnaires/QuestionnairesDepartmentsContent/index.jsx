...
const { Item } = Form;
const QuestionnairesDepartmentsContent = () => {
  ...
  const [size, setSize] = useState(
    Cookies.get("pagination-size-questionnaire-departments")
      ? JSON.parse(Cookies.get("pagination-size-questionnaire-departments"))
      : 20
  );
  const [query, setQuery] = useState({ name: "" });
  const { loading, DepartmentsRender } = useSelector((state) => state.global);

  const Departments = useSelector(
    (state) => state.questionnaire.departments
  );
  ...
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(departmentsVisibility(data_));
    },
    [dispatch]
  );
  ...
  if (Departments?.items) {
    data = Departments?.items?.map((dataObj, i) => ({
      num:
        Departments?.size * Departments?.page + i + 1 - Departments?.size,
      id: dataObj?.id,
      name: dataObj?.name,
      code:dataObj?.code,
  ...
  }, [dispatch, page, DepartmentsRender, query, size]);
  const updateSize = (newSize) => {
    setSize(newSize);
    Cookies.set(
      "pagination-size-questionnaire-departments",
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteDepartments(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
...
export default QuestionnairesDepartmentsContent;
