...
const QuestionnairesCustomersContent = () => {
  ...
  const [size, setSize] = useState(
    Cookies.get("pagination-size-questionnaire-customers")
      ? JSON.parse(Cookies.get("pagination-size-questionnaire-customers"))
      : 20,
  );
  const [query, setQuery] = useState({ name: "" });
  const { loading, CustomersRender } = useSelector((state) => state.global);

  const Customers = useSelector((state) => state.questionnaire.customers);
  ...
  let data = [];
  if (Customers?.items) {
    data = Customers?.items?.map((dataObj, i) => ({
      num: Customers?.size * Customers?.page + i + 1 - Customers?.size,
      id: dataObj?.id,
      name: dataObj?.name,
      customerNumber: dataObj?.customerNumber,
      tin: dataObj?.tin,
  ...
  const updateSize = (newSize) => {
    setSize(newSize);
    Cookies.set(
      "pagination-size-questionnaire-customers",
