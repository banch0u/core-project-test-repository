...
const QuestionnairesDocumentTypeContent = () => {
  ...
  const { loading, militaryStaffsRender } = useSelector(
    (state) => state.global
  );

  const militaryStaffs = useSelector(
    (state) => state.questionnaire.militaryStaffs
  );
  const paginationLength = setPaginationLength(
    militaryStaffs?.count,
    militaryStaffs?.size
  );

  const onSubmit = useCallback(
    async (data) => {
      dispatch(addMilitarystaffs({ ...data, category: typeSelect }));
    },
    [dispatch, typeSelect]
  );
  const onEdit = useCallback(
    (id, record) => {
      ...
      dispatch(editMilitarystaffs(data));
    },
    [dispatch, typeSelect]
  );
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(militaryStaffsVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (militaryStaffs?.items) {
    data = militaryStaffs?.items?.map((dataObj, i) => ({
      num:
        militaryStaffs?.size * militaryStaffs?.page +
        i +
        1 -
        militaryStaffs?.size,
      id: dataObj?.id,
  ...
  useEffect(() => {
    ...
    dispatch(getMilitarystaffs(data));
  }, [dispatch, page, militaryStaffsRender, typeSelect, size, query]);
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteMilitarystaffs(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
