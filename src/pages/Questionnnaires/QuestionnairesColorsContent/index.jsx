...
const { Item } = Form;
const QuestionnairesColorsContent = () => {
  ...
  const [size, setSize] = useState(
    Cookies.get("pagination-size-questionnaire-colors")
      ? JSON.parse(Cookies.get("pagination-size-questionnaire-colors"))
      : 20
  );
  const [query, setQuery] = useState({ name: "" });
  const { loading, colorRender } = useSelector((state) => state.global);

  const color = useSelector((state) => state.questionnaire.color);
  const paginationLength = setPaginationLength(color?.count, color?.size);
  ...
  let data = [];
  if (color?.items) {
    data = color?.items?.map((dataObj, i) => ({
      num: color?.size * color?.page + i + 1 - color?.size,
      id: dataObj?.id,
      name: dataObj?.name,
  ...
  const updateSize = (newSize) => {
    ...
    Cookies.set(
      "pagination-size-questionnaire-colors",
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteColor(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
