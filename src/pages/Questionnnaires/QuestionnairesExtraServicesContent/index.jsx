...
const QuestionnairesExtraServicesContent = () => {
  ...
  const extraServices = useSelector(
    (state) => state.questionnaire.extraServices
  );
  ...
  const onSubmit = useCallback(
    async (data) => {
      dispatch(addExtraServices({ ...data }));
    },
    [dispatch]
  );
  const onEdit = useCallback(
    (id, record) => {
      ...
      dispatch(editExtraServices(data));
    },
    [dispatch]
  );
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(extraServicesVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (extraServices?.items) {
    data = extraServices?.items?.map((dataObj, i) => ({
      num:
        extraServices?.size * extraServices?.page + i + 1 - extraServices?.size,
      id: dataObj?.id,
      name: dataObj?.name,
  ...
  useEffect(() => {
    ...
    dispatch(getExtraServices(data));
...
export default QuestionnairesExtraServicesContent;
