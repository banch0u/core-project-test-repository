...
const QuestionnairesDocumentTypeContent = () => {
  ...
  const { loading, honorarytitlesRender } = useSelector(
    (state) => state.global
  );

  const honorarytitles = useSelector(
    (state) => state.questionnaire.honorarytitles
  );
  const paginationLength = setPaginationLength(
    honorarytitles?.count,
    honorarytitles?.size
  );

  const onSubmit = useCallback(
    async (data) => {
      dispatch(addHonorarytitles({ ...data, category: typeSelect }));
    },
    [dispatch, typeSelect]
  );
  ...
  let data = [];
  if (honorarytitles?.items) {
    data = honorarytitles?.items?.map((dataObj, i) => ({
      num:
        honorarytitles?.size * honorarytitles?.page +
        i +
        1 -
        honorarytitles?.size,
      id: dataObj?.id,
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteHonorarytitles(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
