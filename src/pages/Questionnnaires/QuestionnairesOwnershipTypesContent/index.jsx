...
const { Item } = Form;
const QuestionnairesOwnershipTypesContent = () => {
  ...
  const [size, setSize] = useState(
    Cookies.get("pagination-size-questionnaire-ownership-types")
      ? JSON.parse(Cookies.get("pagination-size-questionnaire-ownership-types"))
      : 20
  );
  const [query, setQuery] = useState({ name: "" });
  const { loading, ownershiptypeRender } = useSelector((state) => state.global);

  const ownershiptype = useSelector(
    (state) => state.questionnaire.ownershiptype
  );
  ...
  const onSubmit = useCallback(
    async (data) => {
      dispatch(addOwnershiptype(data));
    },
    [dispatch]
  );
  const onEdit = useCallback(
    (id, record) => {
      ...
      dispatch(editOwnershiptype(data));
    },
    [dispatch]
  );
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(ownershiptypesVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (ownershiptype?.items) {
    data = ownershiptype?.items?.map((dataObj, i) => ({
      num:
        ownershiptype?.size * ownershiptype?.page + i + 1 - ownershiptype?.size,
      id: dataObj?.id,
      name: dataObj?.name,
  ...
  useEffect(() => {
    ...
    dispatch(getOwnershiptype(data));
  }, [dispatch, page, ownershiptypeRender, query, size]);
  const updateSize = (newSize) => {
    setSize(newSize); // Update state
    Cookies.set(
      "pagination-size-questionnaire-ownership-types",
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteOwnershiptype(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
...
export default QuestionnairesOwnershipTypesContent;
