...
const { Item } = Form;
const QuestionnairesCrushReasonsContent = () => {
  ...
  const [size, setSize] = useState(
    Cookies.get("pagination-size-questionnaire-crush-reasons")
      ? JSON.parse(Cookies.get("pagination-size-questionnaire-crush-reasons"))
      : 20
  );
  const [query, setQuery] = useState({ name: "" });
  const { loading, crushReasonsRender } = useSelector((state) => state.global);

  const crushReasons = useSelector((state) => state.questionnaire.crushReasons);
  ...
  const onEdit = useCallback(
    (id, record) => {
      ...
      dispatch(editCrushReasons(data));
    },
    [dispatch]
  );
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(crushReasonsVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (crushReasons?.items) {
    data = crushReasons?.items?.map((dataObj, i) => ({
      num: crushReasons?.size * crushReasons?.page + i + 1 - crushReasons?.size,
      id: dataObj?.id,
      name: dataObj?.name,
  ...
  useEffect(() => {
    ...
    dispatch(getCrushReasons(data));
  }, [dispatch, page, crushReasonsRender, query, size]);
  const updateSize = (newSize) => {
    setSize(newSize); // Update state
    Cookies.set(
      "pagination-size-questionnaire-crush-reasons",
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteCrushReasons(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
...
export default QuestionnairesCrushReasonsContent;
