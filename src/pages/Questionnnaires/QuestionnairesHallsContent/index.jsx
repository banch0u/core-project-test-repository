...
import { Form, Input, InputNumber, Layout } from "antd";
...
const QuestionnairesDocumentTypeContent = () => {
  ...
  const { loading, hallsRender } = useSelector((state) => state.global);

  const halls = useSelector((state) => state.questionnaire.halls);
  const paginationLength = setPaginationLength(halls?.count, halls?.size);

  const onSubmit = useCallback(
    async (data) => {
      dispatch(addHalls({ ...data }));
    },
    [dispatch]
  );
  const onEdit = useCallback(
    (id, record) => {
      const data = {
        ...
        personCapacity: record?.personCapacity,
      };
      dispatch(editHalls(data));
    },
    [dispatch]
  );
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(hallsVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (halls?.items) {
    data = halls?.items?.map((dataObj, i) => ({
      num: halls?.size * halls?.page + i + 1 - halls?.size,
      id: dataObj?.id,
      name: dataObj?.name,
      location: dataObj?.location,
      personCapacity: dataObj?.personCapacity,
  ...
  }, [dispatch, page, hallsRender, size, query]);
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteHalls(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
