...
const { Item } = Form;
const QuestionnairesOilFieldsContent = () => {
  ...
  const [size, setSize] = useState(
    Cookies.get("pagination-size-questionnaire-oil-fields")
      ? JSON.parse(Cookies.get("pagination-size-questionnaire-oil-fields"))
      : 20
  );
  const [query, setQuery] = useState({ name: "" });
  const { loading, oilFieldsRender } = useSelector((state) => state.global);

  const oilFields = useSelector((state) => state.questionnaire.oilFields);
  ...
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(oilFieldsVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (oilFields?.items) {
    data = oilFields?.items?.map((dataObj, i) => ({
      num: oilFields?.size * oilFields?.page + i + 1 - oilFields?.size,
      id: dataObj?.id,
      name: dataObj?.name,
  ...
  useEffect(() => {
    ...
    dispatch(getOilFields(data));
  }, [dispatch, page, oilFieldsRender, query, size]);
  const updateSize = (newSize) => {
    setSize(newSize); // Update state
    Cookies.set(
      "pagination-size-questionnaire-oil-fields",
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteOilFields(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
...
export default QuestionnairesOilFieldsContent;
