...
const { Item } = Form;
const QuestionnairesRepairmentWorkTypesContent = () => {
  ...
  const [size, setSize] = useState(
    Cookies.get("pagination-size-quetionnaire-repairment-work-types")
      ? JSON.parse(
          Cookies.get("pagination-size-quetionnaire-repairment-work-types")
        )
      : 20
  ...
  const { loading, repairmentWorkTypesRender } = useSelector(
    (state) => state.global
  );

  const repairmentWorkTypes = useSelector(
    (state) => state.questionnaire.repairmentWorkTypes
  );
  const paginationLength = setPaginationLength(
    repairmentWorkTypes?.count,
    repairmentWorkTypes?.size
  );

  const onSubmit = useCallback(
    async (data) => {
      dispatch(addRepairmentWorkTypes(data));
    },
    [dispatch]
  );
  const onEdit = useCallback(
    (id, record) => {
      ...
      dispatch(editRepairmentWorkTypes(data));
    },
    [dispatch]
  );
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(repairmentWorkTypesVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (repairmentWorkTypes?.items) {
    data = repairmentWorkTypes?.items?.map((dataObj, i) => ({
      num:
        repairmentWorkTypes?.size * repairmentWorkTypes?.page +
        i +
        1 -
        repairmentWorkTypes?.size,
      id: dataObj?.id,
  ...
  useEffect(() => {
    ...
    dispatch(getRepairmentWorkTypes(data));
  }, [dispatch, page, repairmentWorkTypesRender, query, size]);
  const updateSize = (newSize) => {
    setSize(newSize); // Update state
    Cookies.set(
      "pagination-size-quetionnaire-repairment-work-types",
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteRepairmentWorkTypes(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
...
export default QuestionnairesRepairmentWorkTypesContent;
