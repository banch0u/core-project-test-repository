...
const QuestionnairesDocumentTypeContent = () => {
  ...
  const { loading, educationPaymentsRender } = useSelector(
    (state) => state.global
  );

  const educationPayments = useSelector(
    (state) => state.questionnaire.educationPayments
  );
  const paginationLength = setPaginationLength(
    educationPayments?.count,
    educationPayments?.size
  );

  const onSubmit = useCallback(
    async (data) => {
      dispatch(addEducationpayments({ ...data, category: typeSelect }));
    },
    [dispatch, typeSelect]
  );
  const onEdit = useCallback(
    (id, record) => {
      ...
      dispatch(editEducationpayments(data));
    },
    [dispatch, typeSelect]
  );
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(educationPaymentsVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (educationPayments?.items) {
    data = educationPayments?.items?.map((dataObj, i) => ({
      num:
        educationPayments?.size * educationPayments?.page +
        i +
        1 -
        educationPayments?.size,
      id: dataObj?.id,
  ...
  useEffect(() => {
    ...
    dispatch(getEducationpayments(data));
  }, [dispatch, page, educationPaymentsRender, typeSelect, size, query]);
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteEducationpayments(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
