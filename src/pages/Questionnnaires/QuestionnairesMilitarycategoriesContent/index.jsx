...
const QuestionnairesDocumentTypeContent = () => {
  ...
  const { loading, militaryCategoriesRender } = useSelector(
    (state) => state.global
  );

  const militaryCategories = useSelector(
    (state) => state.questionnaire.militaryCategories
  );
  const paginationLength = setPaginationLength(
    militaryCategories?.count,
    militaryCategories?.size
  );

  const onSubmit = useCallback(
    async (data) => {
      dispatch(addMilitarycategories({ ...data, category: typeSelect }));
    },
    [dispatch, typeSelect]
  );
  ...
  let data = [];
  if (militaryCategories?.items) {
    data = militaryCategories?.items?.map((dataObj, i) => ({
      num:
        militaryCategories?.size * militaryCategories?.page +
        i +
        1 -
        militaryCategories?.size,
      id: dataObj?.id,
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteMilitarycategories(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
