...
const { Item } = Form;
const QuestionnairesPenaltyTypesContent = () => {
  ...
  const [size, setSize] = useState(
    Cookies.get("pagination-size-questionnaire-penalty-types")
      ? JSON.parse(Cookies.get("pagination-size-questionnaire-penalty-types"))
      : 20
  );
  const [query, setQuery] = useState({ name: "" });
  const { loading, penaltyTypesRender } = useSelector((state) => state.global);

  const penaltyTypes = useSelector((state) => state.questionnaire.penaltyTypes);
  ...
  const onEdit = useCallback(
    (id, record) => {
      ...
      dispatch(editPenaltyTypes(data));
    },
    [dispatch]
  );
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(penaltyTypesVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (penaltyTypes?.items) {
    data = penaltyTypes?.items?.map((dataObj, i) => ({
      num: penaltyTypes?.size * penaltyTypes?.page + i + 1 - penaltyTypes?.size,
      id: dataObj?.id,
      name: dataObj?.name,
  ...
  useEffect(() => {
    ...
    dispatch(getPenaltyTypes(data));
  }, [dispatch, page, penaltyTypesRender, query, size]);
  const updateSize = (newSize) => {
    setSize(newSize); // Update state
    Cookies.set(
      "pagination-size-questionnaire-penalty-types",
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deletePenaltyTypes(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
...
export default QuestionnairesPenaltyTypesContent;
