...
const { Item } = Form;
const QuestionnairesChassisTypesContent = () => {
  ...
  const [size, setSize] = useState(
    Cookies.get("pagination-size-questionnaire-chassis-types")
      ? JSON.parse(Cookies.get("pagination-size-questionnaire-chassis-types"))
      : 20
  );
  const [query, setQuery] = useState({ name: "" });
  const { loading, chassisTypeRender } = useSelector((state) => state.global);

  const chassisTypes = useSelector((state) => state.questionnaire.chassisTypes);
  ...
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(chassisTypeVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (chassisTypes?.items) {
    data = chassisTypes?.items?.map((dataObj, i) => ({
      num: chassisTypes?.size * chassisTypes?.page + i + 1 - chassisTypes?.size,
      id: dataObj?.id,
      name: dataObj?.name,
  ...
  useEffect(() => {
    ...
    dispatch(getChassisTypes(data));
  }, [dispatch, page, chassisTypeRender, query, size]);
  const updateSize = (newSize) => {
    setSize(newSize); // Update state
    Cookies.set(
      "pagination-size-questionnaire-chassis-types",
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteChassisType(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
...
export default QuestionnairesChassisTypesContent;
