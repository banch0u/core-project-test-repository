...
const QuestionnairesDocumentTypeContent = () => {
  ...
  const { loading, reprimandTypesRender } = useSelector(
    (state) => state.global
  );

  const reprimandTypes = useSelector(
    (state) => state.questionnaire.reprimandTypes
  );
  const paginationLength = setPaginationLength(
    reprimandTypes?.count,
    reprimandTypes?.size
  );

  const onSubmit = useCallback(
    async (data) => {
      dispatch(addReprimandtypes({ ...data, category: typeSelect }));
    },
    [dispatch, typeSelect]
  );
  const onEdit = useCallback(
    (id, record) => {
      ...
      dispatch(editReprimandtypes(data));
    },
    [dispatch, typeSelect]
  );
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(reprimandTypesVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (reprimandTypes?.items) {
    data = reprimandTypes?.items?.map((dataObj, i) => ({
      num:
        reprimandTypes?.size * reprimandTypes?.page +
        i +
        1 -
        reprimandTypes?.size,
      id: dataObj?.id,
  ...
  useEffect(() => {
    ...
    dispatch(getReprimandtypes(data));
  }, [dispatch, page, reprimandTypesRender, typeSelect, size, query]);
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteReprimandtypes(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
