...
const QuestionnairesApplicationFormContent = () => {
  ...
  const [size, setSize] = useState(
    Cookies.get("pagination-size-questionnaire-application-form")
      ? JSON.parse(
          Cookies.get("pagination-size-questionnaire-application-form")
        )
      : 20
  ...
  const { loading, applicationFormRender } = useSelector(
    (state) => state.global
  );

  const applicationForms = useSelector(
    (state) => state.questionnaire.applicationForms
  );

  const paginationLength = setPaginationLength(
    applicationForms?.count,
    applicationForms?.size
  );
  ...
  let data = [];
  if (applicationForms?.items) {
    data = applicationForms?.items?.map((dataObj, i) => ({
      num:
        applicationForms?.size * applicationForms?.page +
        i +
        1 -
        applicationForms?.size,
      id: dataObj?.id,
  ...
  const updateSize = (newSize) => {
    ...
    Cookies.set(
      "pagination-size-questionnaire-application-form",
