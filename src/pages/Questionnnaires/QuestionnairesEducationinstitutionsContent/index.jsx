...
const QuestionnairesDocumentTypeContent = () => {
  ...
  const { loading, educationinstitutionsRender } = useSelector(
    (state) => state.global
  );

  const educationinstitutions = useSelector(
    (state) => state.questionnaire.educationinstitutions
  );
  const paginationLength = setPaginationLength(
    educationinstitutions?.count,
    educationinstitutions?.size
  );

  const onSubmit = useCallback(
    async (data) => {
      dispatch(addEducationinstitutions({ ...data, category: typeSelect }));
    },
    [dispatch, typeSelect]
  );
  const onEdit = useCallback(
    (id, record) => {
      ...
      dispatch(editEducationinstitutions(data));
    },
    [dispatch, typeSelect]
  );
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(educationinstitutionsVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (educationinstitutions?.items) {
    data = educationinstitutions?.items?.map((dataObj, i) => ({
      num:
        educationinstitutions?.size * educationinstitutions?.page +
        i +
        1 -
        educationinstitutions?.size,
      id: dataObj?.id,
  ...
  useEffect(() => {
    ...
    dispatch(getEducationinstitutions(data));
  }, [dispatch, page, educationinstitutionsRender, typeSelect, size, query]);
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteEducationinstitutions(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
