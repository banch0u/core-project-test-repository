...
const QuestionnairesDocumentTypeContent = () => {
  ...
  const { loading, generalstructuretypesRender } = useSelector(
    (state) => state.global
  );

  const generalstructuretypes = useSelector(
    (state) => state.questionnaire.generalstructuretypes
  );
  const paginationLength = setPaginationLength(
    generalstructuretypes?.count,
    generalstructuretypes?.size
  );

  const onSubmit = useCallback(
    async (data) => {
      dispatch(addGeneralstructuretypes({ ...data, category: typeSelect }));
    },
    [dispatch, typeSelect]
  );
  const onEdit = useCallback(
    (id, record) => {
      ...
      dispatch(editGeneralstructuretypes(data));
    },
    [dispatch, typeSelect]
  );
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(generalstructuretypesVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (generalstructuretypes?.items) {
    data = generalstructuretypes?.items?.map((dataObj, i) => ({
      num:
        generalstructuretypes?.size * generalstructuretypes?.page +
        i +
        1 -
        generalstructuretypes?.size,
      id: dataObj?.id,
  ...
  useEffect(() => {
    ...
    dispatch(getGeneralstructuretypes(data));
  }, [dispatch, page, generalstructuretypesRender, typeSelect, size, query]);
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteGeneralstructuretypes(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
