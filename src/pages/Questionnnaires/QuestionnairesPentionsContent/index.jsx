...
const { Item } = Form;
const QuestionnairesPentionsContent = () => {
  ...
  const [size, setSize] = useState(
    Cookies.get("pagination-size-questionnaire-pentions")
      ? JSON.parse(Cookies.get("pagination-size-questionnaire-pentions"))
      : 20
  );
  const [query, setQuery] = useState({ name: "" });
  const { loading, PentionsRender } = useSelector((state) => state.global);

  const Pentions = useSelector(
    (state) => state.questionnaire.pentions
  );
  ...
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(pentionsVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (Pentions?.items) {
    data = Pentions?.items?.map((dataObj, i) => ({
      num:
        Pentions?.size * Pentions?.page + i + 1 - Pentions?.size,
      id: dataObj?.id,
      name: dataObj?.name,
  ...
  useEffect(() => {
    ...
    dispatch(getPentions(data));
  }, [dispatch, page, PentionsRender, query, size]);
  const updateSize = (newSize) => {
    setSize(newSize);
    Cookies.set(
      "pagination-size-questionnaire-pentions",
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deletePentions(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
...
export default QuestionnairesPentionsContent;
