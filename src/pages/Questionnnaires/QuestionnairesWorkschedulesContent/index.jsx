...
const QuestionnairesDocumentTypeContent = () => {
  ...
  const { loading, workschedulesRender } = useSelector((state) => state.global);

  const workschedules = useSelector(
    (state) => state.questionnaire.workschedules
  );
  ...
  const onSubmit = useCallback(
    async (data) => {
      dispatch(addWorkschedules({ ...data, category: typeSelect }));
    },
    [dispatch, typeSelect]
  );
  const onEdit = useCallback(
    (id, record) => {
      ...
      dispatch(editWorkschedules(data));
    },
    [dispatch, typeSelect]
  );
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(workschedulesVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (workschedules?.items) {
    data = workschedules?.items?.map((dataObj, i) => ({
      num:
        workschedules?.size * workschedules?.page + i + 1 - workschedules?.size,
      id: dataObj?.id,
      name: dataObj?.name,
  ...
  useEffect(() => {
    ...
    dispatch(getWorkschedules(data));
  }, [dispatch, page, workschedulesRender, typeSelect, size, query]);
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteWorkschedules(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
