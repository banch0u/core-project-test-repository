...
const { Item } = Form;
const QuestionnairesMeasurementTypesContent = () => {
  ...
  let data = [];
  if (measurementtypes?.items) {
    data = measurementtypes?.items?.map((dataObj, i) => ({
      num:
        measurementtypes?.size * measurementtypes?.page +
        i +
        1 -
        measurementtypes?.size,
      id: dataObj?.id,
  ...
  useEffect(() => {
    ...
    dispatch(getMeasurementtypes(data));
  }, [dispatch, page, measurementtypesRender, size, query]);
  const updateSize = (newSize) => {
    setSize(newSize); // Update state
    Cookies.set(
      "pagination-size-questionnaired-measurement-types",
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteMeasurementtypes(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
...
export default QuestionnairesMeasurementTypesContent;
