...
const QuestionnairesDocumentTypeContent = () => {
  ...
  const { loading, academicdegreesRender } = useSelector(
    (state) => state.global
  );

  const academicdegrees = useSelector(
    (state) => state.questionnaire.academicdegrees
  );
  const paginationLength = setPaginationLength(
    academicdegrees?.count,
    academicdegrees?.size
  );

  const onSubmit = useCallback(
    async (data) => {
      dispatch(addAcademicdegrees({ ...data, category: typeSelect }));
    },
    [dispatch, typeSelect]
  );
  const onEdit = useCallback(
    (id, record) => {
      ...
      dispatch(editAcademicdegrees(data));
    },
    [dispatch, typeSelect]
  );
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(academicdegreesVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (academicdegrees?.items) {
    data = academicdegrees?.items?.map((dataObj, i) => ({
      num:
        academicdegrees?.size * academicdegrees?.page +
        i +
        1 -
        academicdegrees?.size,
      id: dataObj?.id,
  ...
  useEffect(() => {
    ...
    dispatch(getAcademicdegrees(data));
  }, [dispatch, page, academicdegreesRender, typeSelect, size, query]);
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteAcademicdegrees(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
