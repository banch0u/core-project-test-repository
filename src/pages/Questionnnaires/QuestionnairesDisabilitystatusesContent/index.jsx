...
const QuestionnairesDocumentTypeContent = () => {
  ...
  const { loading, disabilitystatusesRender } = useSelector(
    (state) => state.global
  );

  const disabilitystatuses = useSelector(
    (state) => state.questionnaire.disabilitystatuses
  );
  const paginationLength = setPaginationLength(
    disabilitystatuses?.count,
    disabilitystatuses?.size
  );

  const onSubmit = useCallback(
    async (data) => {
      dispatch(addDisabilitystatuses({ ...data, category: typeSelect }));
    },
    [dispatch, typeSelect]
  );
  ...
  let data = [];
  if (disabilitystatuses?.items) {
    data = disabilitystatuses?.items?.map((dataObj, i) => ({
      num:
        disabilitystatuses?.size * disabilitystatuses?.page +
        i +
        1 -
        disabilitystatuses?.size,
      id: dataObj?.id,
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteDisabilitystatuses(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
