...
const { Item } = Form;
const QuestionnairesBrandsContent = () => {
  ...
  const [size, setSize] = useState(
    Cookies.get("pagination-size-questionnaire-brands")
      ? JSON.parse(Cookies.get("pagination-size-questionnaire-brands"))
      : 20
  );
  const [query, setQuery] = useState({ name: "" });
  const { loading, brandRender } = useSelector((state) => state.global);

  const brands = useSelector((state) => state.questionnaire.brands);
  const paginationLength = setPaginationLength(brands?.count, brands?.size);
  ...
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(brandVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (brands?.items) {
    data = brands?.items?.map((dataObj, i) => ({
      num: brands?.size * brands?.page + i + 1 - brands?.size,
      id: dataObj?.id,
      name: dataObj?.name,
  ...
  }, [dispatch, page, brandRender, query, size]);
  const updateSize = (newSize) => {
    setSize(newSize); // Update state
    Cookies.set(
      "pagination-size-questionnaire-brands",
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteBrand(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
...
export default QuestionnairesBrandsContent;
