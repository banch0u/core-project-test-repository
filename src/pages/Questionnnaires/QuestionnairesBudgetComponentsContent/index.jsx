...
const { Item } = Form;
const QuestionnairesBudgetComponentsContent = () => {
  ...
  const [size, setSize] = useState(
    Cookies.get("pagination-size-questionnaire-budgetcomponents")
      ? JSON.parse(Cookies.get("pagination-size-questionnaire-budgetcomponents"))
      : 20
  );
  const [query, setQuery] = useState({ name: "" });
  const { loading, BudgetComponentsRender } = useSelector((state) => state.global);

  const BudgetComponents = useSelector(
    (state) => state.questionnaire.budgetComponents
  );
  const paginationLength = setPaginationLength(
    BudgetComponents?.count,
    BudgetComponents?.size
  );
  ...
  let data = [];
  if (BudgetComponents?.items) {
    data = BudgetComponents?.items?.map((dataObj, i) => ({
      num:
        BudgetComponents?.size * BudgetComponents?.page + i + 1 - BudgetComponents?.size,
      id: dataObj?.id,
      name: dataObj?.name,
  ...
  const updateSize = (newSize) => {
    setSize(newSize);
    Cookies.set(
      "pagination-size-questionnaire-budgetcomponents",
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteBudgetComponents(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
