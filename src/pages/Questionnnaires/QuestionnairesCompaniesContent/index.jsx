...
const QuestionnairesDocumentTypeContent = () => {
  ...
  const { loading, companiesRender } = useSelector((state) => state.global);

  const companies = useSelector((state) => state.questionnaire.companies);
  ...
  const onEdit = useCallback(
    (id, record) => {
      const data = {
        ...
        isCurrentCompany: record?.isCurrentCompany,
      };
  ...
  let data = [];
  if (companies?.items) {
    data = companies?.items?.map((dataObj, i) => ({
      num: companies?.size * companies?.page + i + 1 - companies?.size,
      id: dataObj?.id,
      name: dataObj?.name,
      fullName: dataObj?.fullName,
      isCurrentCompany: dataObj?.isCurrentCompany,
