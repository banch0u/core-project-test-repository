...
const QuestionnairesDocumentTypeContent = () => {
  ...
  const { loading, documentTypesRender } = useSelector((state) => state.global);

  const documentTypes = useSelector(
    (state) => state.questionnaire.documentTypes
  );
  ...
  const onSubmit = useCallback(
    async (data) => {
      dispatch(addDocumentTypes({ ...data, category: typeSelect }));
    },
    [dispatch, typeSelect]
  );
  ...
  let data = [];
  if (documentTypes?.items) {
    data = documentTypes?.items?.map((dataObj, i) => ({
      num:
        documentTypes?.size * documentTypes?.page + i + 1 - documentTypes?.size,
      id: dataObj?.id,
      name: dataObj?.name,
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            <div className={style.table_header}>
              ...
              <div className={style.buttons}>
                <Select
                  ...
                  <Option value={1}>Daxil olan sənədlər</Option>
                  <Option value={2}>Xaric olan sənədlər</Option>
                </Select>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteDocumentTypes(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
