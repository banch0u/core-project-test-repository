...
const { Item } = Form;
const QuestionnairesOwnersContent = () => {
  ...
  const [size, setSize] = useState(
    Cookies.get("pagination-size-questionnaire-owners")
      ? JSON.parse(Cookies.get("pagination-size-questionnaire-owners"))
      : 20
  );
  const [query, setQuery] = useState({ name: "" });
  const { loading, OwnersRender } = useSelector((state) => state.global);

  const Owners = useSelector(
    (state) => state.questionnaire.owners
  );
  ...
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(OwnersVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (Owners?.items) {
    data = Owners?.items?.map((dataObj, i) => ({
      num:
        Owners?.size * Owners?.page + i + 1 - Owners?.size,
      id: dataObj?.id,
      name: dataObj?.name,
  ...
  }, [dispatch, page, OwnersRender, query, size]);
  const updateSize = (newSize) => {
    setSize(newSize); // Update state
    Cookies.set(
      "pagination-size-questionnaire-owners",
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteOwners(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
...
export default QuestionnairesOwnersContent;
