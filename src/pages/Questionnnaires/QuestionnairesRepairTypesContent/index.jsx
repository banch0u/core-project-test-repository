...
const { Item } = Form;
const QuestionnairesRepairTypesContent = () => {
  ...
  const [size, setSize] = useState(
    Cookies.get("pagination-size-questionnaired-repair-types")
      ? JSON.parse(Cookies.get("pagination-size-questionnaired-repair-types"))
      : 20
  );
  ...
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(repairtypesVisibility(data_));
    },
    [dispatch]
  );
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteRepairtypes(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
...
export default QuestionnairesRepairTypesContent;
