...
const QuestionnairesDocumentTypeContent = () => {
  ...
  const { loading, areasRender } = useSelector((state) => state.global);

  const areas = useSelector((state) => state.questionnaire.areas);
  const paginationLength = setPaginationLength(areas?.count, areas?.size);

  const onSubmit = useCallback(
    async (data) => {
      dispatch(addAreas({ ...data }));
    },
    [dispatch]
  );
  ...
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(areasVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (areas?.items) {
    data = areas?.items?.map((dataObj, i) => ({
      num: areas?.size * areas?.page + i + 1 - areas?.size,
      id: dataObj?.id,
      name: dataObj?.name,
  ...
  }, [dispatch, page, areasRender, size, query]);
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteAreas(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
