...
const QuestionnairesProjectsContent = () => {
  ...
  const [size, setSize] = useState(
    Cookies.get("pagination-size-questionnaire-projects")
      ? JSON.parse(Cookies.get("pagination-size-questionnaire-projects"))
      : 20
  );
  const [query, setQuery] = useState({ name: "" });
  const { loading, ProjectsRender } = useSelector((state) => state.global);

  const Projects = useSelector(
    (state) => state.questionnaire.projects
  );
  ...
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(projectsVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (Projects?.items) {
    data = Projects?.items?.map((dataObj, i) => ({
      num:
        Projects?.size * Projects?.page + i + 1 - Projects?.size,
      id: dataObj?.id,
      name: dataObj?.name,
  ...
  }, [dispatch, page, ProjectsRender, query, size]);
  const updateSize = (newSize) => {
    setSize(newSize);
    Cookies.set(
      "pagination-size-questionnaire-projects",
...
export default QuestionnairesProjectsContent;
