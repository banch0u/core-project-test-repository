...
const { Item } = Form;
const QuestionnairesOrdersContent = () => {
  ...
  const { loading, ordersRender } = useSelector((state) => state.global);

  const orders = useSelector((state) => state.questionnaire.orders);
  const paginationLength = setPaginationLength(orders?.count, orders?.size);
  ...
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(ordersVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (orders?.items) {
    data = orders?.items?.map((dataObj, i) => ({
      num: orders?.size * orders?.page + i + 1 - orders?.size,
      id: dataObj?.id,
      name: dataObj?.name,
  ...
  }, [dispatch, page, ordersRender, size, query]);
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteOrders(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
...
export default QuestionnairesOrdersContent;
