...
const QuestionnairesDocumentTypeContent = () => {
  ...
  const { loading, specializationsRender } = useSelector(
    (state) => state.global
  );

  const specializations = useSelector(
    (state) => state.questionnaire.specializations
  );
  ...
  const onSubmit = useCallback(
    async (data) => {
      dispatch(addSpecializations({ ...data, category: typeSelect }));
    },
    [dispatch, typeSelect]
  );
  const onEdit = useCallback(
    (id, record) => {
      ...
      dispatch(editSpecializations(data));
    },
    [dispatch, typeSelect]
  );
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(specializationsVisibility(data_));
    },
    [dispatch]
  );
  ...
  if (specializations?.items) {
    data = specializations?.items?.map((dataObj, i) => ({
      num:
        specializations?.size * specializations?.page +
        i +
        1 -
        specializations?.size,
      id: dataObj?.id,
  ...
  useEffect(() => {
    ...
    dispatch(getSpecializations(data));
  }, [dispatch, page, specializationsRender, typeSelect, size, query]);
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteSpecializations(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
