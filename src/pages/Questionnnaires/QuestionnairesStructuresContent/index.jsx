...
const { Item } = Form;
const QuestionnairesStructuresContent = () => {
  ...
  const [size, setSize] = useState(
    Cookies.get("pagination-size-questionnaire-structures")
      ? JSON.parse(Cookies.get("pagination-size-questionnaire-structures"))
      : 20
  );
  const [query, setQuery] = useState({ name: "" });
  const { loading, structureRender } = useSelector((state) => state.global);

  const structures = useSelector((state) => state.questionnaire.structures);
  ...
  let data = [];
  if (structures?.items) {
    data = structures?.items?.map((dataObj, i) => ({
      num: structures?.size * structures?.page + i + 1 - structures?.size,
      id: dataObj?.id,
      name: dataObj?.name,
  ...
  const updateSize = (newSize) => {
    ...
    Cookies.set(
      "pagination-size-questionnaire-structures",
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteStructure(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
