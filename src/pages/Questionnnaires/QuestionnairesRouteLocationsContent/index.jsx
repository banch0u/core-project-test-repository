...
const { Item } = Form;
const QuestionnairesRouteLocationsContent = () => {
  ...
  const [size, setSize] = useState(
    Cookies.get("pagination-size-questionnaire-routelocations")
      ? JSON.parse(Cookies.get("pagination-size-questionnaire-routelocations"))
      : 20
  );
  const [query, setQuery] = useState({ name: "" });
  const { loading, RouteLocationsRender } = useSelector((state) => state.global);

  const RouteLocations = useSelector(
    (state) => state.questionnaire.routeLocations
  );
  ...
  let data = [];
  if (RouteLocations?.items) {
    data = RouteLocations?.items?.map((dataObj, i) => ({
      num:
        RouteLocations?.size * RouteLocations?.page + i + 1 - RouteLocations?.size,
      id: dataObj?.id,
      name: dataObj?.name,
  ...
  const updateSize = (newSize) => {
    setSize(newSize);
    Cookies.set(
      "pagination-size-questionnaire-routelocations",
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteRouteLocations(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
