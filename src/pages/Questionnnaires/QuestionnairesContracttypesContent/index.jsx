...
const { Item } = Form;
const QuestionnairesContracttypesContent = () => {
  ...
  const { loading, contracttypesRender } = useSelector((state) => state.global);

  const contracttypes = useSelector(
    (state) => state.questionnaire.contracttypes
  );
  ...
  const onSubmit = useCallback(
    async (data) => {
      dispatch(addContracttypes({ ...data }));
    },
    [dispatch]
  );
  const onEdit = useCallback(
    (id, record) => {
      ...
      dispatch(editContracttypes(data));
    },
    [dispatch]
  );
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(contracttypesVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (contracttypes?.items) {
    data = contracttypes?.items?.map((dataObj, i) => ({
      num:
        contracttypes?.size * contracttypes?.page + i + 1 - contracttypes?.size,
      id: dataObj?.id,
      name: dataObj?.name,
      code: dataObj?.code,
  ...
  useEffect(() => {
    ...
    dispatch(getContracttypes(data));
  }, [dispatch, page, contracttypesRender, size, query]);
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteContracttypes(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
...
export default QuestionnairesContracttypesContent;
