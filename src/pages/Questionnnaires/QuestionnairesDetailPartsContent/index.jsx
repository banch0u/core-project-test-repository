...
const { Item } = Form;
const QuestionnairesDetailPartsContent = () => {
  ...
  const [size, setSize] = useState(
    Cookies.get("pagination-size-questionnaired-detail-parts")
      ? JSON.parse(Cookies.get("pagination-size-questionnaired-detail-parts"))
      : 20
  );
  const [query, setQuery] = useState({ name: "" });

  const { loading, detailpartsRender } = useSelector((state) => state.global);

  const detailparts = useSelector((state) => state.questionnaire.detailparts);
  ...
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(detailpartsVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (detailparts?.items) {
    data = detailparts?.items?.map((dataObj, i) => ({
      num: detailparts?.size * detailparts?.page + i + 1 - detailparts?.size,
      id: dataObj?.id,
      name: dataObj?.name,
  ...
  useEffect(() => {
    ...
    dispatch(getDetailparts(data));
  }, [dispatch, page, detailpartsRender, size, query]);
  const updateSize = (newSize) => {
    setSize(newSize); // Update state
    Cookies.set(
      "pagination-size-questionnaired-detail-parts",
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteDetailparts(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
...
export default QuestionnairesDetailPartsContent;
