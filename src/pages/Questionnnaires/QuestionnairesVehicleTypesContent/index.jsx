...
const QuestionnairesVehicleTypesContent = () => {
  ...
  const [size, setSize] = useState(
    Cookies.get("pagination-size-questionnaire-vehicle-types")
      ? JSON.parse(Cookies.get("pagination-size-questionnaire-vehicle-types"))
      : 20
  );
  const [query, setQuery] = useState({ name: "" });
  const { loading, vehicletypeRender } = useSelector((state) => state.global);

  const vehicletype = useSelector((state) => state.questionnaire.vehicletype);
  ...
  let data = [];
  if (vehicletype?.items) {
    data = vehicletype?.items?.map((dataObj, i) => ({
      num: vehicletype?.size * vehicletype?.page + i + 1 - vehicletype?.size,
      id: dataObj?.id,
      name: dataObj?.name,
  ...
  }, [dispatch, page, vehicletypeRender, query, size]);
  const updateSize = (newSize) => {
    setSize(newSize); // Update state
    Cookies.set(
      "pagination-size-questionnaire-vehicle-types",
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteVehicletype(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
...
export default QuestionnairesVehicleTypesContent;
