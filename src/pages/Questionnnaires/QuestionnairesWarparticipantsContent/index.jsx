...
const QuestionnairesDocumentTypeContent = () => {
  ...
  const { loading, warparticipantsRender } = useSelector(
    (state) => state.global
  );

  const warparticipants = useSelector(
    (state) => state.questionnaire.warparticipants
  );
  const paginationLength = setPaginationLength(
    warparticipants?.count,
    warparticipants?.size
  );

  const onSubmit = useCallback(
    async (data) => {
      dispatch(addWarparticipants({ ...data, category: typeSelect }));
    },
    [dispatch, typeSelect]
  );
  const onEdit = useCallback(
    (id, record) => {
      ...
      dispatch(editWarparticipants(data));
    },
    [dispatch, typeSelect]
  );
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(warparticipantsVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (warparticipants?.items) {
    data = warparticipants?.items?.map((dataObj, i) => ({
      num:
        warparticipants?.size * warparticipants?.page +
        i +
        1 -
        warparticipants?.size,
      id: dataObj?.id,
  ...
  useEffect(() => {
    ...
    dispatch(getWarparticipants(data));
  }, [dispatch, page, warparticipantsRender, typeSelect, size, query]);
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteWarparticipants(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
