...
const QuestionnairesEngineTypesContent = () => {
  ...
  const [size, setSize] = useState(
    Cookies.get("pagination-size-questionnaire-engine-types")
      ? JSON.parse(Cookies.get("pagination-size-questionnaire-engine-types"))
      : 20
  );
  const [query, setQuery] = useState({ name: "" });
  const { loading, enginetypeRender } = useSelector((state) => state.global);

  const enginetype = useSelector((state) => state.questionnaire.enginetype);
  ...
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(enginetypesVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (enginetype?.items) {
    data = enginetype?.items?.map((dataObj, i) => ({
      num: enginetype?.size * enginetype?.page + i + 1 - enginetype?.size,
      id: dataObj?.id,
      name: dataObj?.name,
  ...
  useEffect(() => {
    ...
    dispatch(getEnginetype(data));
  }, [dispatch, page, enginetypeRender, query, size]);
  const updateSize = (newSize) => {
    setSize(newSize); // Update state
    Cookies.set(
      "pagination-size-questionnaire-engine-types",
      ...
      <Layout className={style.layout}>
        <Content className={style.content}>
          <header className={style.header}>
            <Button onClick={onClickModal} enginetype="green">
...
export default QuestionnairesEngineTypesContent;
