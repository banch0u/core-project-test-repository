...
const { Item } = Form;
const QuestionnairesVehicleGroupsContent = () => {
  ...
  const [size, setSize] = useState(
    Cookies.get("pagination-size-questionnaired-vehicle-groups")
      ? JSON.parse(Cookies.get("pagination-size-questionnaired-vehicle-groups"))
      : 20
  );
  ...
  const onSubmit = useCallback(
    async (data) => {
      dispatch(addVehicleGroups(data));
    },
    [dispatch]
  );
  const onEdit = useCallback(
    (id, record) => {
      ...
      dispatch(editVehicleGroups(data));
    },
    [dispatch]
  );
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(vehicleGroupsVisibility(data_));
    },
    [dispatch]
  );
  ...
  const updateSize = (newSize) => {
    ...
    Cookies.set(
      "pagination-size-questionnaired-vehicle-groups",
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteVehicleGroups(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
...
export default QuestionnairesVehicleGroupsContent;
