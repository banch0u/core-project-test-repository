...
const { Item } = Form;
const QuestionnairesContractTopicsContent = () => {
  ...
  const [size, setSize] = useState(
    Cookies.get("pagination-size-questionnaire-contracttopics")
      ? JSON.parse(Cookies.get("pagination-size-questionnaire-contracttopics"))
      : 20
  );
  const [query, setQuery] = useState({ name: "" });
  const { loading, ContractTopicsRender } = useSelector((state) => state.global);

  const ContractTopics = useSelector(
    (state) => state.questionnaire.contractTopics
  );
  const paginationLength = setPaginationLength(
    ContractTopics?.count,
    ContractTopics?.size
  );
  ...
  let data = [];
  if (ContractTopics?.items) {
    data = ContractTopics?.items?.map((dataObj, i) => ({
      num:
        ContractTopics?.size * ContractTopics?.page + i + 1 - ContractTopics?.size,
      id: dataObj?.id,
      name: dataObj?.name,
  ...
  const updateSize = (newSize) => {
    setSize(newSize);
    Cookies.set(
      "pagination-size-questionnaire-contracttopics",
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteContractTopics(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
