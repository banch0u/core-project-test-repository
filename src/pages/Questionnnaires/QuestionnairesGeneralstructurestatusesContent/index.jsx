...
const QuestionnairesDocumentTypeContent = () => {
  ...
  const { loading, generalstructurestatusesRender } = useSelector(
    (state) => state.global
  );

  const generalstructurestatuses = useSelector(
    (state) => state.questionnaire.generalstructurestatuses
  );
  const paginationLength = setPaginationLength(
    generalstructurestatuses?.count,
    generalstructurestatuses?.size
  );

  const onSubmit = useCallback(
    async (data) => {
      dispatch(addGeneralstructurestatuses({ ...data, category: typeSelect }));
    },
    [dispatch, typeSelect]
  );
  const onEdit = useCallback(
    (id, record) => {
      ...
      dispatch(editGeneralstructurestatuses(data));
    },
    [dispatch, typeSelect]
  );
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(generalstructurestatusesVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (generalstructurestatuses?.items) {
    data = generalstructurestatuses?.items?.map((dataObj, i) => ({
      num:
        generalstructurestatuses?.size * generalstructurestatuses?.page +
        i +
        1 -
        generalstructurestatuses?.size,
      id: dataObj?.id,
  ...
  useEffect(() => {
    ...
    dispatch(getGeneralstructurestatuses(data));
  }, [dispatch, page, generalstructurestatusesRender, typeSelect, size, query]);
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            <div className={style.table_header}>
              <h2>Ümumi struktur statusları</h2>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteGeneralstructurestatuses(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
