...
const QuestionnairesDocumentTypeContent = () => {
  ...
  const { loading, militarygroupsRender } = useSelector(
    (state) => state.global
  );

  const militarygroups = useSelector(
    (state) => state.questionnaire.militarygroups
  );
  const paginationLength = setPaginationLength(
    militarygroups?.count,
    militarygroups?.size
  );

  const onSubmit = useCallback(
    async (data) => {
      dispatch(addMilitarygroups({ ...data, category: typeSelect }));
    },
    [dispatch, typeSelect]
  );
  const onEdit = useCallback(
    (id, record) => {
      ...
      dispatch(editMilitarygroups(data));
    },
    [dispatch, typeSelect]
  );
  const onStatusChange = useCallback(
    (data, checked) => {
      ...
      dispatch(militarygroupsVisibility(data_));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (militarygroups?.items) {
    data = militarygroups?.items?.map((dataObj, i) => ({
      num:
        militarygroups?.size * militarygroups?.page +
        i +
        1 -
        militarygroups?.size,
      id: dataObj?.id,
  ...
  useEffect(() => {
    ...
    dispatch(getMilitarygroups(data));
  }, [dispatch, page, militarygroupsRender, typeSelect, size, query]);
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteMilitarygroups(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
