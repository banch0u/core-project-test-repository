...
import {
  addDrivingcategories,
  deleteDrivingcategories,
  drivingcategoriesVisibility,
  editDrivingcategories,
...
const QuestionnairesDocumentTypeContent = () => {
  ...
  const { loading, drivingcategoriesRender } = useSelector(
    (state) => state.global
  );

  const drivingcategories = useSelector(
    (state) => state.questionnaire.drivingcategories
  );

  const paginationLength = setPaginationLength(
    drivingcategories?.count,
    drivingcategories?.size
  );
  const onSubmit = useCallback(
    async (data) => {
      dispatch(addDrivingcategories({ ...data }));
    },
    [dispatch]
  );
  ...
  let data = [];
  if (drivingcategories?.items) {
    data = drivingcategories?.items?.map((dataObj, i) => ({
      num:
        drivingcategories?.size * drivingcategories?.page +
        i +
        1 -
        drivingcategories?.size,
      id: dataObj?.id,
        ...
        <Layout className={style.layout1}>
          <Content className={style.content}>
            <div className={style.table_header}>
              <h2>Sürücülük kateqoriyaları</h2>
            ...
            <DeleteModal
              ...
              width={280}>
              <Delete
                onDelete={() => dispatch(deleteDrivingcategories(id))}
                onCancel={() => dispatch(setDeleteModalVisible(false))}
                value={"Soraqçanı"}
              />
            </DeleteModal>
