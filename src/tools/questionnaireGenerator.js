...
////////// === CONFIG === //////////
const questionnaireName = "Owners"; // change as needed
const constantColumnName = "Mülkiyyətçi";
const endpointName = "/owners";
///////////////////////////////////

const upperName = questionnaireName.toUpperCase();
const pathConstA = `QUESTIONNAIRES_${upperName}`;
const pathConstB = `SETTINGS_QUESTIONNAIRES_${upperName}`;
...
export const ${pathConstB} = SETTINGS_QUESTIONNAIRES + "${endpointName}";
// ---- end generated ----\n`;
...
try {
  ...
  if (!routesPath) {
    ...
    if (pathImportMatch) {
      ...
      if (!names.includes(pathConstB)) { names.push(pathConstB); changed = true; }
      if (changed) {
      ...
      const safeImport = `import { ${pathConstA}, ${pathConstB} } from "../utils/path";\n`;
    ...
    const docStartRegex = /<Route\b[^>]*\bpath=(?:["'])questionnaires(?:["'])[^>]*>/m;
    const settingsStartRegex = /<Route\b[^>]*\bpath=(?:["'])settings\/questionnaires(?:["'])[^>]*>/m;

    const docRouteMarker = ``;
    const setRouteMarker = ``;

    const routeEntryDocument = `\n${docRouteMarker}\n        <Route\n          path={${pathConstA}}\n          element={<Questionnaires${questionnaireName}Content />}\n        />\n`;
    const routeEntrySettings = `\n${setRouteMarker}\n        <Route\n          path={${pathConstB}}\n          element={<Questionnaires${questionnaireName}Content />}\n        />\n`;

    // Insert into Document Circulation Routes parent block
    const resDoc = insertRouteIntoParentBlock(routesContent, docStartRegex, routeEntryDocument, docRouteMarker);
    if (resDoc.inserted) {
    ...
    const resSet = insertRouteIntoParentBlock(routesContent, settingsStartRegex, routeEntrySettings, setRouteMarker);
    if (resSet.inserted) {
      ...
      const settingsBlockRegex = /<Route\s*\n\s*path="settings\/questionnaires"[\s\S]*?<\/Route>/m;
      if (settingsBlockRegex.test(routesContent)) {
        routesContent = routesContent.replace(settingsBlockRegex, (m) => {
          const idx = m.lastIndexOf("</Route>");
          if (idx !== -1) {
