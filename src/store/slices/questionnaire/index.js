...
export const getFields = createAsyncThunk(
  ...
  async (data, { dispatch }) => {
    try {
      dispatch(setLoading(true));
      const response = await Services.getFields(
        data.size,
        data.page,
        data.query,
        data.visibility
...
export const getFieldsAll = createAsyncThunk(
  "/getFieldsAll",
  async (visibility, { dispatch }) => {
    try {
      dispatch(setLoading(true));
      const response = await Services.getFieldsAll(visibility);
      dispatch(setLoading(false));
      return response?.data?.data;
    } catch (error) {
      errorMessage(error.response?.data?.message);
...
export const addFields = createAsyncThunk(
  ...
  async (data, { dispatch }) => {
    try {
      dispatch(setLoading(true));
      await Services.addFields(data);
      dispatch(setLoading(false));
      dispatch(setFieldsRender((prev) => !prev));
      dispatch(setViewModalVisible(true));
    } catch (error) {
      errorMessage(error.response?.data?.message);
      dispatch(setLoading(false));
...
export const editFields = createAsyncThunk(
  ...
  async (data, { dispatch }) => {
    try {
      dispatch(setLoading(true));
      const response = await Services.editFields(data);
      dispatch(setLoading(false));
      dispatch(setFieldsRender((prev) => !prev));
      return response?.data;
    } catch (error) {
...
export const deleteFields = createAsyncThunk(
  ...
  async (id, { dispatch }) => {
    try {
      dispatch(setLoading(true));
      await Services.deleteFields(id);
      dispatch(setLoading(false));
      dispatch(setDeleteModalVisible(false));
      dispatch(setFieldsRender((prev) => !prev));
    } catch (error) {
...
export const fieldsVisibility = createAsyncThunk(
  "/fieldsVisibility",
  async (data, { dispatch }) => {
    try {
      dispatch(setLoading(true));
      const response = await Services.fieldsVisibility(data);
      dispatch(setLoading(false));
      dispatch(setFieldsRender((prev) => !prev));
      return response?.data;
    } catch (error) {
...
export const questionnaire = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(getFieldsAll.fulfilled, (state, { payload }) => {
      state.fieldsAll = payload;
    });
