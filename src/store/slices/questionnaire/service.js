...
// ---- generated by questionnaireGenerator: Pentions ----
  static getPentions = async (size, page, query, visibility) => {
    const response = await api.get(`/pentions/${size}/page/${page}?visibility=${visibility}&name=${query?.name}`);
    return response?.data;
  };
  static getPentionsAll = async (visibility) => {
    const response = await api.get(`/pentions?visibility=${visibility}`);
    return response?.data;
  };
  static addPentions = async (value) => {
    const response = await api.post("/pentions", value);
    return response?.data;
  };
  static editPentions = async (data) => {
    const response = await api.put(`/pentions/${data?.id}`, data);
    return response?.data;
  };
  static deletePentions = async (id) => {
    const response = await api.delete(`/pentions/${id}`);
    return response?.data;
  };
  static pentionsVisibility = async (data) => {
    const response = await api.patch(`/pentions/${data?.id}/visibility/${data?.checked}`);
    return response?.data;
  };
// ---- end generated ----
