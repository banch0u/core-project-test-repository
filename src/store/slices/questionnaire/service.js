...
static getFields = async (size, page, query, visibility) => {
    return await api.get(
      `/fields/${size}/page/${page}?visibility=${visibility}&name=${query?.name}`
    );
  };

  static getFieldsAll = async (visibility) => {
    return await api.get(`/fields?visibility=${visibility}`);
  };

  static addFields = async (data) => {
    return await api.post(`/fields`, data, {
      headers: { "Content-Type": "application/json" },
    });
  };

  static editFields = async (value) => {
    return await api.put(`/fields/${value?.id}`, value);
  };

  static deleteFields = async (id) => {
    return await api.delete(`/fields/${id}`);
  };

  static fieldsVisibility = async (data) => {
    return await api.patch(`/fields/${data?.id}/visibility/${data?.checked}`);
  }
static getBarrel = async (size, page, query, visibility) => {
    return await api.get(
      `/barrels/${size}/page/${page}?visibility=${visibility}&name=${query?.name}`
    );
  };

  static getBarrelAll = async (visibility) => {
    return await api.get(`/barrels?visibility=${visibility}`);
  };

  static addBarrel = async (data) => {
    return await api.post(`/barrels`, data, {
      headers: { "Content-Type": "application/json" },
    });
  };

  static editBarrel = async (value) => {
    return await api.put(`/barrels/${value?.id}`, value);
  };

  static deleteBarrel = async (id) => {
    return await api.delete(`/barrels/${id}`);
  };

  static barrelVisibility = async (data) => {
    return await api.patch(`/barrels/${data?.id}/visibility/${data?.checked}`);
  };
...
// ---- generated by questionnaireGenerator: DefaultAgreementPlans ----
  static getDefaultAgreementPlans = async (size, page, query, visibility) => {
    const response = await api.get(`/defaultagreementplans/${size}/page/${page}?visibility=${visibility}&name=${query?.name}`);
    return response?.data;
  };
