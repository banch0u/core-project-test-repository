...
const VendorCategoryTreeSelect = ({ value, onChange, ...rest }) => {
  const dispatch = useDispatch();

  const vendorCategoriesAll = useSelector(
    (state) => state.questionnaire.vendorCategoriesAll,
  );

  useEffect(() => {
    if (!vendorCategoriesAll?.length) {
  ...
  return (
    <TreeSelect
      value={value}
      onChange={onChange}
      treeData={mapCategoriesToTree(vendorCategoriesAll ?? [])}
      showSearch
      allowClear
